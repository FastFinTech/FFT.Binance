...
namespace FFT.Binance.MarketDataStreams
{
  ...
  /// </summary>
  public sealed partial class BinanceMarketDataClient : AsyncDisposeBase, IAsyncDisposable
  {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BinanceMarketDataClient"/>
    ...
    /// <inheritdoc/>
    protected override ValueTask CustomDisposeAsync() => new(_workTask);
    ...
    private async Task Work()
    {
      ...
      try
      {
        ...
        while (true)
        {
          ...
          // or b) receive a signal that subscriptions have been added or canceled.

          // Setup a cancellation token that is linked between the
          // "DisposedToken" and the signal.
          using var signalTokenSource = new CancellationTokenSource();
          using var linkedTokenSource = CancellationTokenSource.CreateLinkedTokenSource(signalTokenSource.Token, DisposedToken);
          _ = _signalEvent.WaitAsync(DisposedToken).ContinueWith(t => signalTokenSource.Cancel(), TaskContinuationOptions.OnlyOnRanToCompletion);

          // This try block will swallow exceptions arising from the signal,
          // allowing us to return to the top of the main loop, but throw all
          // other exceptions.
          try
          {
            while (true)
            {
              ...
              var result = await ws.ReceiveAsync(buffer.GetMemory(BUFFER_SIZE), DisposedToken);
              buffer.Advance(result.Count);

              while (!result.EndOfMessage)
              {
                result = await ws.ReceiveAsync(buffer.GetMemory(BUFFER_SIZE), DisposedToken);
                buffer.Advance(result.Count);
              }
          ...
          catch (OperationCanceledException x) when (x.CancellationToken == linkedTokenSource.Token) { }
        }
      }
      catch (Exception x)
      {
    ...
    private abstract class Stream : IAsyncDisposable
    {
      ...
      public virtual ValueTask Initiate(BinanceMarketDataClient connection) => default;
    ...
    private sealed class FullDepthStream : Stream
    {
      ...
      public override async ValueTask Initiate(BinanceMarketDataClient connection)
      {
        var orderBookResponse = await connection.GetOrderBook(StreamInfo.Name.Substring(StreamInfo.Name.IndexOf('@') + 1), 1000);
    ...
    {
      private readonly BinanceMarketDataClient _connection;
      private readonly Channel<object> _channel = Channel.CreateUnbounded<object>();

      private long _disposed = 0;

      public Subscription(BinanceMarketDataClient connection, StreamInfo streamInfo)
      {
      ...
      public void Handle(object message) => _channel.Writer.TryWrite(message);
